...
public class Linker{
    public static JsonNode combine(Reference reference) throws ReferenceException, JsonMappingException, JsonProcessingException{
        if(reference.getSource().isMissingNode()){
            if(reference.getFragment().equals(""))
                throw new ReferenceException("could not resolve missed ref with uri - '" + reference.getUri() + "'");
        ...
        ((ObjectNode) currentNode).set("resultOfDereference", schemeResolver.dereferenceResolve(reference.getSource()));
